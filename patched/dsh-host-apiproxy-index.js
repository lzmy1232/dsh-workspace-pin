...
/** PowerShell expression resolving the Windows system Notepad without PATH lookup. */
const WINDOWS_NOTEPAD = "(Join-Path $env:SystemRoot 'System32\\notepad.exe')";
/**
...
/** Open one Windows-resolvable path for the requested intent. */
async function openWindowsPath(path, signal, intent, run) {
	const quotedEditorPath = powershellLiteral(`"${path}"`);
	await run("powershell.exe", [
		"-NoProfile",
		"-Command",
		intent === "text-editor" ? `Start-Process -FilePath ${WINDOWS_NOTEPAD} -ArgumentList ${quotedEditorPath}` : `Invoke-Item -LiteralPath ${powershellLiteral(path)}`
	], signal);
}
/** Translate a WSL path before handing it to the Windows desktop. */
async function openWslPath(path, signal, intent, run) {
	const translated = await run("wslpath", ["-w", path], signal);
	signal.throwIfAborted();
	const windowsPath = translated.stdout.replace(/[\r\n]+$/, "");
	if (windowsPath === "") throw new Error("wslpath returned no Windows path");
	await openWindowsPath(windowsPath, signal, intent, run);
}
...
async function openNativePathWithIntent(path, signal, intent, internals = {}) {
	...
	if (platform === "win32") {
		await openWindowsPath(path, signal, intent, run);
		return;
	}
	if (platform === "linux") {
		if (wsl) {
...
/**
* Open a text document for editing; macOS and Windows bypass the file-type
* association so an absent or unsuitable YAML association cannot consume the
* gesture.
...
/** Cap for the cold title fallback's whole-log read (keeps listing bounded). */
const TITLE_FALLBACK_MAX_BYTES = 2 * 1024 * 1024;
/** Per-run memo of cold-folded titles, keyed by log identity (createdAt + size). */
const coldTitleMemo = /* @__PURE__ */ new Map();
/**
* Fold a cold session's title directly from its persisted log when the
* projection cache is stale and holds no title for it. Whole-log read capped
* by {@link TITLE_FALLBACK_MAX_BYTES}; the result is memoized per log identity
* so repeated listings re-read nothing. Fail-soft by contract: any failure
* returns undefined and the row degrades to the no-title display.
...
* @returns the folded title, or undefined.
*/
async function coldTitleFor(ctx, persistence, meta, signal) {
	const location = persistence.locate?.(meta);
	if (location === void 0) return void 0;
	const size = (await stat(location.path)).size;
	if (size > TITLE_FALLBACK_MAX_BYTES) return void 0;
	const key = `${meta.id}|${meta.createdAt}|${size}`;
	const memo = coldTitleMemo.get(key);
	if (memo !== void 0) return memo;
	const { events } = await persistence.readFrom(meta.id, 0, signal);
	const registry = ctx.get("sessionProjections");
	if (registry === void 0) return void 0;
	const restored = registry.restore({}, events, 0);
	const title = restored.snapshot.values.title;
	const result = typeof title === "string" && title !== "" ? title : void 0;
	coldTitleMemo.set(key, result);
	return result;
}
		...
		if (persistence !== void 0) {
			...
			for (let offset = 0; offset < cold.length; offset += COLD_SUMMARY_BATCH_SIZE) {
				...
				const settled = await Promise.allSettled(batch.map(async (meta) => {
					...
					let served = projections;
					if (served === void 0 || typeof served.values.title !== "string" || served.values.title === "") {
						try {
							const title = await coldTitleFor(ctx, persistence, meta, signal);
							if (title !== void 0) served = {
								...served === void 0 ? { asOfSeq: -1 } : served,
								values: {
									...served?.values,
									title
								}
							...
							/* v8 ignore next -- fail-soft: title degradation never breaks listing */
						}
					}
					return {
						...summary,
						...served === void 0 ? {} : { projections: served }
					};
				}));
				const summaries = [];
				let rejected = false;
