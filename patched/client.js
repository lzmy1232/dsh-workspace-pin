window.__ModuleLoader__.load({
	...
	factory: (require) => {
			...
			return (0, _deepseek_ai_dsh_client_runtime_client.defineStore)({
				...
				actions: {
					...
					togglePin: (d, sessionId) => {
						if (d.pinnedSessions === void 0) d.pinnedSessions = {};
						if (d.pinnedSessions[sessionId] !== void 0) delete d.pinnedSessions[sessionId];
						else d.pinnedSessions[sessionId] = Date.now();
					},
		...
		*/
		function groupByWorkspace(list, workspaces, archived, ungroupedOrder, pinnedIds) {
			const groups = [];
			...
			const stray = list.ids.map((id) => list.byId[id]).filter((s) => s !== void 0 && !accounted.has(s.id) && !pinnedIds.has(s.id) && sessionVisible(s, list.current, archived));
		...
		*/
		function deriveGroups(list, workspaces, archivedSessionIds, view, pinnedIds) {
			...
			const groups = [];
			for (const g of groupByWorkspace(list, workspaces, archived, view.ungroupedOrder, pinnedIds)) {
		...
		*/
		function deriveFlat(list, archivedSessionIds, pinnedIds) {
			...
			for (const id of list.ids) {
				...
				if (s === void 0 || !sessionVisible(s, list.current, archived) || pinnedIds.has(id)) continue;
				rows.push(s);
			}
			rows.sort(byRecency);
		...
		/**
		* Derive the global pinned section: user-pinned sessions, newest pin first.
		* Blank and archived sessions never pin; pinned sessions are excluded from
		* the regular groups (see {@link deriveGroups} / {@link deriveFlat}) and live
		* only here, above every group in both the tree and flat list.
		* @param list - sessions list snapshot.
		* @param archivedSessionIds - registry-global archive set (members never pin).
		* @param pinnedSessions - view-store pin map (id -> pin time ms); may be
		* undefined before the first pin action rehydrates a pre-existing persisted
		* view state that predates the pinnedSessions field.
		* @returns pinned session nodes in render order.
		*/
		function derivePinned(list, archivedSessionIds, pinnedSessions) {
			const archived = new Set(archivedSessionIds);
			const pinned = pinnedSessions === void 0 ? {} : pinnedSessions;
			const entries = [];
			for (const id of list.ids) {
				const summary = list.byId[id];
				const pinnedAt = pinned[id];
				if (summary === void 0 || pinnedAt === void 0 || summary.blank || !sessionVisible(summary, list.current, archived)) continue;
				entries.push({
					pinnedAt,
					summary
				});
			}
			entries.sort((a, b) => b.pinnedAt - a.pinnedAt || (a.summary.id < b.summary.id ? -1 : 1));
			const descendants = (0, _deepseek_ai_dsh_client_runtime_client.indexSubagentDescendants)(list.byId);
			return entries.map((entry) => sessionNode(entry.summary, descendants));
		}
		/**
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:C:\Users\t4wefan\code\github\deepseek-harness-desktop\packages\client\ui-workspace\src\client\rows\Rows.module.css.mjs
		const css$2 = ".PD2ZSG_projectRow,.PD2ZSG_sessionRow{cursor:pointer;user-select:none;color:var(--dsw-alias-label-primary);border-radius:8px;align-items:center;gap:6px;padding:0 8px;display:flex}.PD2ZSG_projectRow:hover,.PD2ZSG_sessionRow:hover,.PD2ZSG_sessionRow.PD2ZSG_selected{background:var(--dsw-alias-interactive-bg-hover)}.PD2ZSG_searchResultRow{box-sizing:border-box;cursor:pointer;text-align:left;width:100%;min-height:48px;color:var(--dsw-alias-label-primary);background:0 0;border:none;border-radius:8px;flex-direction:column;align-items:stretch;padding:4px 8px;display:flex}.PD2ZSG_searchResultRow:hover,.PD2ZSG_searchResultRow.PD2ZSG_selected{background:var(--dsw-alias-interactive-bg-hover)}.PD2ZSG_searchResultHeading{align-items:center;min-width:0;display:flex}.PD2ZSG_searchResultTitle{text-overflow:ellipsis;white-space:nowrap;min-width:0;margin-left:4px;font-size:14px;line-height:20px;overflow:hidden}.PD2ZSG_searchResultMeta{align-items:center;gap:6px;min-width:0;margin-left:20px;display:flex}.PD2ZSG_searchResultWorkspace,.PD2ZSG_searchResultSnippet{text-overflow:ellipsis;white-space:nowrap;font-size:12px;line-height:17px;overflow:hidden}.PD2ZSG_searchResultWorkspace{max-width:40%;color:var(--dsw-alias-label-tertiary);flex:none}.PD2ZSG_searchResultSnippet{min-width:0;color:var(--dsw-alias-label-secondary);flex:1}.PD2ZSG_projectRow{box-sizing:border-box;align-items:center;height:34px}.PD2ZSG_projectRow .PD2ZSG_rowActions{height:20px}.PD2ZSG_sessionRow{height:32px;animation:PD2ZSG_row-in .15s var(--ds-ease-in-out);gap:0}.PD2ZSG_sessionRow .PD2ZSG_title{margin:0 6px 0 4px}.PD2ZSG_flatSessionRowWithoutStatus .PD2ZSG_title{margin-left:0}@keyframes PD2ZSG_row-in{0%{opacity:0}}.PD2ZSG_slot{width:16px;height:20px;color:var(--dsw-alias-label-tertiary);flex:none;justify-content:center;align-items:center;display:inline-flex}.PD2ZSG_visuallyHidden{clip:rect(0 0 0 0);white-space:nowrap;width:1px;height:1px;position:absolute;overflow:hidden}.PD2ZSG_folderActive{color:var(--dsw-alias-state-business-primary)}.PD2ZSG_projectRow .PD2ZSG_chevron{display:none}.PD2ZSG_projectRow:hover .PD2ZSG_chevron{display:inline-flex}.PD2ZSG_projectRow:hover .PD2ZSG_folder{display:none}.PD2ZSG_arrow{transition:transform .15s var(--ds-ease-in-out)}.PD2ZSG_arrowOpen{transform:rotate(90deg)}.PD2ZSG_projectText{flex-direction:column;flex:1;gap:2px;min-width:0;display:flex}.PD2ZSG_title{text-overflow:ellipsis;white-space:nowrap;min-width:0;font-size:14px;line-height:20px;overflow:hidden}.PD2ZSG_renameInput{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-button-elevated-fill);min-width:0;color:inherit;border-radius:4px;outline:none;padding:0 2px;font-size:14px;line-height:20px}.PD2ZSG_sessionRow .PD2ZSG_title{flex:1}.PD2ZSG_meta{text-overflow:ellipsis;white-space:nowrap;color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:20px;overflow:hidden}.PD2ZSG_time{color:var(--dsw-alias-label-tertiary);flex:none;font-size:12px;line-height:20px}.PD2ZSG_dot{flex:none}.PD2ZSG_rowActions{flex:none;align-items:center;gap:12px;display:none}.PD2ZSG_projectRow:hover .PD2ZSG_rowActions,.PD2ZSG_sessionRow:hover .PD2ZSG_rowActions,.PD2ZSG_projectRow.PD2ZSG_menuOpen .PD2ZSG_rowActions,.PD2ZSG_sessionRow.PD2ZSG_menuOpen .PD2ZSG_rowActions{display:inline-flex}.PD2ZSG_sessionRow:hover .PD2ZSG_time,.PD2ZSG_sessionRow.PD2ZSG_menuOpen .PD2ZSG_time{display:none}.PD2ZSG_projectRow.PD2ZSG_menuOpen,.PD2ZSG_sessionRow.PD2ZSG_menuOpen{background:var(--dsw-alias-interactive-bg-hover)}.PD2ZSG_sessionRow.PD2ZSG_dropBefore,.PD2ZSG_sessionRow.PD2ZSG_dropAfter{position:relative}.PD2ZSG_sessionRow.PD2ZSG_dropBefore:before,.PD2ZSG_sessionRow.PD2ZSG_dropAfter:after{content:\"\";z-index:1;background:linear-gradient(55deg, transparent calc(50% - 1px), var(--dsw-alias-state-business-primary) calc(50% - 1px) calc(50% + 1px), transparent calc(50% + 1px)) 0 0 / 5px 7px no-repeat, linear-gradient(125deg, transparent calc(50% - 1px), var(--dsw-alias-state-business-primary) calc(50% - 1px) calc(50% + 1px), transparent calc(50% + 1px)) 0 5px / 5px 7px no-repeat, linear-gradient(var(--dsw-alias-state-business-primary) 0 0) 4px 5px / calc(100% - 4px) 2px no-repeat;pointer-events:none;height:12px;position:absolute;left:0;right:4px}.PD2ZSG_sessionRow.PD2ZSG_dropBefore:before{top:-7px}.PD2ZSG_sessionRow.PD2ZSG_dropAfter:after{bottom:-7px}.PD2ZSG_hoverContent{flex-direction:column;gap:8px;display:flex}.PD2ZSG_hoverTitle{color:#fff;overflow-wrap:break-word;font-size:14px;line-height:20px}.PD2ZSG_hoverPath{color:#cfd3d6;word-break:break-all;font-size:12px;line-height:16px}.PD2ZSG_hoverTime{color:#cfd3d6;font-size:12px;line-height:16px}.PD2ZSG_hoverStatus{color:#adb2b8;align-items:center;gap:8px;font-size:12px;line-height:20px;display:flex}.PD2ZSG_iconButton{cursor:pointer;width:16px;height:16px;color:var(--dsw-alias-label-tertiary);background:0 0;border:none;border-radius:4px;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.PD2ZSG_iconButton:hover{color:var(--dsw-alias-label-primary)}.PD2ZSG_chevron{color:var(--dsw-alias-label-caption)}@media (prefers-reduced-motion:reduce){.PD2ZSG_sessionRow,.PD2ZSG_arrow{transition:none;animation:none}}.PD2ZSG_pinGlyph{color:var(--dsw-alias-label-tertiary);flex:none;display:inline-flex}.PD2ZSG_pinGlyphPinned{color:var(--dsw-alias-state-business-primary)}.PD2ZSG_pinMarker{width:12px;height:12px;color:var(--dsw-alias-label-tertiary);flex:none;align-items:center;justify-content:center;display:inline-flex}";
		...
		var Rows_module_css_default = {
			"dropAfter": "PD2ZSG_dropAfter",
			"projectText": "PD2ZSG_projectText",
			"hoverPath": "PD2ZSG_hoverPath",
			"dropBefore": "PD2ZSG_dropBefore",
			"hoverTime": "PD2ZSG_hoverTime",
			"rowActions": "PD2ZSG_rowActions",
			"searchResultTitle": "PD2ZSG_searchResultTitle",
			"searchResultRow": "PD2ZSG_searchResultRow",
			"sessionRow": "PD2ZSG_sessionRow",
			"searchResultMeta": "PD2ZSG_searchResultMeta",
			"hoverContent": "PD2ZSG_hoverContent",
			"iconButton": "PD2ZSG_iconButton",
			"folder": "PD2ZSG_folder",
			"visuallyHidden": "PD2ZSG_visuallyHidden",
			"arrowOpen": "PD2ZSG_arrowOpen",
			"menuOpen": "PD2ZSG_menuOpen",
			"projectRow": "PD2ZSG_projectRow",
			"selected": "PD2ZSG_selected",
			"arrow": "PD2ZSG_arrow",
			"hoverStatus": "PD2ZSG_hoverStatus",
			"searchResultHeading": "PD2ZSG_searchResultHeading",
			"searchResultSnippet": "PD2ZSG_searchResultSnippet",
			"title": "PD2ZSG_title",
			"slot": "PD2ZSG_slot",
			"flatSessionRowWithoutStatus": "PD2ZSG_flatSessionRowWithoutStatus",
			"chevron": "PD2ZSG_chevron",
			"time": "PD2ZSG_time",
			"searchResultWorkspace": "PD2ZSG_searchResultWorkspace",
			"hoverTitle": "PD2ZSG_hoverTitle",
			"renameInput": "PD2ZSG_renameInput",
			"row-in": "PD2ZSG_row-in",
			"meta": "PD2ZSG_meta",
			"folderActive": "PD2ZSG_folderActive",
			"dot": "PD2ZSG_dot",
			"pinGlyph": "PD2ZSG_pinGlyph",
			"pinGlyphPinned": "PD2ZSG_pinGlyphPinned",
			"pinMarker": "PD2ZSG_pinMarker"
		};
		//#endregion
		...
		*/
		...
		/** Inline pushpin glyph (the icon set has no pin; 24-unit Material push_pin scaled to size). */
		function PinGlyph({ size = 14, className }) {
		...
		* @param props.pinned - the row is shown in the global pinned section.
		* @param props.onTogglePin - pin/unpin a session by id.
		* @param props.t - the browser root's locale seat.
		* @returns the session row.
		*/
		function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, drag, flat = false, pinned = false, onTogglePin, t }) {
			const row = node;
			...
			const sessionMenuItems = [
				...
				...(row.blank || onTogglePin === void 0 ? [] : [{
					id: pinned ? "unpin" : "pin",
					label: pinned ? t("menu.unpin") : t("menu.pin"),
					icon: (0, react_jsx_runtime.jsx)(PinGlyph, { size: 14 })
				}])
			];
			return (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.HoverCard, {
				anchor: (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						pinned && (0, react_jsx_runtime.jsx)("span", {
							className: Rows_module_css_default.pinMarker,
							children: (0, react_jsx_runtime.jsx)(PinGlyph, { size: 12 })
						}),
						(0, react_jsx_runtime.jsx)("span", {
							className: Rows_module_css_default.title,
							children: title
						...
						!row.blank && (0, react_jsx_runtime.jsxs)("span", {
							...
							children: [onTogglePin !== void 0 && (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: Rows_module_css_default.iconButton,
								"aria-label": pinned ? t("actions.unpin.aria", { name: title }) : t("actions.pin.aria", { name: title }),
								...
								children: (0, react_jsx_runtime.jsx)(PinGlyph, {
									size: 14,
									className: clsx(Rows_module_css_default.pinGlyph, pinned && Rows_module_css_default.pinGlyphPinned)
								})
								...
								onSelect: (id) => {
									...
									if (id === "pin" || id === "unpin") onTogglePin?.(node.id);
								},
								portal: true,
								closeOnPointerLeave: true,
								anchor: (0, react_jsx_runtime.jsx)("button", {
		...
		/**
		* The global pinned section: user-pinned sessions, newest pin first, above
		* every group in the tree and flat list. Renders nothing when nothing is
		* pinned. Rows reuse the ordinary session row with the pinned affordances.
		* @param props.pinnedRows - derived pinned session nodes (render order).
		* @param props.current - selected session id.
		* @param props.now - epoch ms for relative-time formatting.
		...
		* @param props.onTogglePin - pin/unpin a session by id.
		* @param props.t - the browser root's locale seat.
		* @returns the pinned section, or null when nothing is pinned.
		*/
		function PinnedSection({ pinnedRows, current, now, open, onRename, onFork, onArchive, onTogglePin, t }) {
			if (pinnedRows.length === 0) return null;
			return (0, react_jsx_runtime.jsxs)("div", {
				className: WorkspaceBrowser_module_css_default.pinnedSection,
				role: "group",
				"aria-label": t("pinned.section"),
				children: [
					(0, react_jsx_runtime.jsxs)("div", {
						className: WorkspaceBrowser_module_css_default.pinnedHeader,
						children: [(0, react_jsx_runtime.jsx)("span", {
							className: Rows_module_css_default.pinGlyph,
							children: (0, react_jsx_runtime.jsx)(PinGlyph, { size: 12 })
						}), (0, react_jsx_runtime.jsx)("span", {
							children: t("pinned.section")
						})]
					}),
					(0, react_jsx_runtime.jsxs)("div", {
						className: WorkspaceBrowser_module_css_default.pinnedList,
						children: pinnedRows.map((node) => (0, react_jsx_runtime.jsx)(SessionNodeItem, {
							node,
							currentId: current,
							now,
							onOpen: open,
		...
		//#endregion
		//#region \0dsh-css:C:\Users\t4wefan\code\github\deepseek-harness-desktop\packages\client\ui-workspace\src\client\WorkspacePicker.module.css.mjs
		const css$1 = "._3te71G_modalAction{min-width:72px}._3te71G_modalError,._3te71G_menuStatus{margin-top:8px;font-size:12px;line-height:18px}._3te71G_modalError{color:var(--dsw-alias-state-error-primary)}._3te71G_menuStatus{color:var(--dsw-alias-label-secondary)}";
		...
		var WorkspacePicker_module_css_default = {
			"modalError": "_3te71G_modalError",
			"menuStatus": "_3te71G_menuStatus",
			"modalAction": "_3te71G_modalAction"
		};
		//#endregion
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:C:\Users\t4wefan\code\github\deepseek-harness-desktop\packages\client\ui-workspace\src\client\WorkspaceBrowser.module.css.mjs
		const css = ".sWxSOG_root{--dsh-session-list-edge-inset:var(--dsh-sidebar-inline-padding);--dsh-session-list-scrollbar-width:8px;--dsh-session-list-scrollbar-offset:2px;box-sizing:border-box;min-height:0;padding-right:var(--dsh-session-list-edge-inset);flex-direction:column;flex:1;display:flex}.sWxSOG_root.sWxSOG_rail{padding-right:0}.sWxSOG_iconButton{cursor:pointer;width:28px;height:28px;color:var(--dsw-alias-label-secondary);background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.sWxSOG_iconButton:hover{background:var(--dsw-alias-interactive-bg-hover)}.sWxSOG_sectionHeader{box-sizing:border-box;height:36px;color:var(--dsw-alias-label-tertiary);border-radius:12px;flex:none;justify-content:flex-end;align-items:center;gap:4px;margin-bottom:4px;padding-left:4px;display:flex;overflow:hidden}.sWxSOG_root:not(.sWxSOG_rail) .sWxSOG_sectionHeader{margin-top:2px;margin-right:-4px}.sWxSOG_sectionLabel{white-space:nowrap;opacity:1;visibility:visible;min-width:0;max-width:45%;transition:max-width .18s var(--ds-ease-in-out), margin-right .18s var(--ds-ease-in-out), opacity .12s var(--ds-ease-in-out), transform .18s var(--ds-ease-in-out), visibility 0s linear;flex:none;line-height:20px;overflow:hidden}.sWxSOG_sectionLabelHidden{opacity:0;visibility:hidden;max-width:0;margin-right:-4px;transition-delay:0s,0s,0s,0s,.18s;transform:translate(-4px)}.sWxSOG_searchSlot{box-sizing:border-box;min-width:0;max-width:28px;transition:max-width .18s var(--ds-ease-in-out), padding-left .18s var(--ds-ease-in-out);flex:1;align-items:center;margin-left:auto;padding-left:0;display:flex}.sWxSOG_searchSlotExpanded{max-width:100%;padding-left:0}.sWxSOG_headerActions{opacity:1;visibility:visible;max-width:60px;transition:max-width .18s var(--ds-ease-in-out), opacity .12s var(--ds-ease-in-out), transform .18s var(--ds-ease-in-out), visibility 0s linear;flex:none;align-items:center;gap:4px;display:flex;overflow:hidden}.sWxSOG_headerActionsHidden{opacity:0;visibility:hidden;pointer-events:none;max-width:0;transition-delay:0s,0s,0s,.18s;transform:translate(4px)}.sWxSOG_search{box-sizing:border-box;cursor:text;width:100%;height:28px;color:var(--dsw-alias-label-secondary);transition:width .18s var(--ds-ease-in-out), padding .18s var(--ds-ease-in-out), border-color .18s var(--ds-ease-in-out), background-color .18s var(--ds-ease-in-out);background:0 0;border:none;border-radius:50%;flex:none;align-items:center;gap:0;margin:0;padding:0;display:flex;overflow:hidden}.sWxSOG_searchExpanded{border:1px solid var(--dsw-alias-border-l2);width:calc(100% + 4px);height:30px;color:var(--dsw-alias-label-caption);background:0 0;border-radius:10px;margin-inline:-2px;padding:0 4px 0 0}.sWxSOG_searchButton{cursor:pointer;width:28px;height:28px;color:inherit;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.sWxSOG_searchExpanded .sWxSOG_searchButton{width:28px;height:30px}.sWxSOG_searchButton:hover{background:var(--dsw-alias-interactive-bg-hover)}.sWxSOG_searchExpanded .sWxSOG_searchButton:hover{background:0 0}.sWxSOG_searchInput{opacity:0;pointer-events:none;width:0;min-width:0;color:var(--dsw-alias-label-primary);transition:opacity .12s var(--ds-ease-in-out);background:0 0;border:none;outline:none;flex:1;font-size:13px;line-height:18px}.sWxSOG_searchExpanded .sWxSOG_searchInput{opacity:1;pointer-events:auto;margin-left:-2px}.sWxSOG_searchInput::placeholder{color:var(--dsw-alias-label-tertiary)}.sWxSOG_clearButton{cursor:pointer;width:24px;height:24px;color:var(--dsw-alias-label-secondary);background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.sWxSOG_clearButton:hover{background:var(--dsw-alias-interactive-bg-hover)}.sWxSOG_rail .sWxSOG_sectionHeader{justify-content:flex-start;gap:0;margin-bottom:12px;padding-left:0}.sWxSOG_rail .sWxSOG_headerActions{max-width:none}.sWxSOG_rail .sWxSOG_iconButton{width:36px;height:36px;color:var(--dsw-alias-label-primary)}.sWxSOG_rail .sWxSOG_search{background:0 0;border-color:#0000;gap:0;width:36px;height:36px;margin:0 0 12px;padding:0}.sWxSOG_rail .sWxSOG_searchButton{width:36px;height:36px;color:var(--dsw-alias-label-primary)}.sWxSOG_rail .sWxSOG_searchButton:hover{background:var(--dsw-alias-interactive-bg-hover)}.sWxSOG_listArea{min-height:0;margin-left:-4px;margin-right:calc(-1 * var(--dsh-session-list-edge-inset));flex-direction:column;flex:1;padding-left:4px;display:flex;overflow:visible}.sWxSOG_rail .sWxSOG_listArea{margin-left:0;margin-right:0;padding-left:0}.sWxSOG_treeBody{flex-direction:column;flex:1;min-height:0;display:flex;position:relative}.sWxSOG_fade{left:0;right:var(--dsh-session-list-edge-inset);background:linear-gradient(to bottom, transparent, var(--dsw-specific-sidebar-fill));pointer-events:none;height:24px;position:absolute;bottom:0}html[data-dsh-desktop-platform=darwin] .sWxSOG_fade,html[data-dsh-desktop-platform=win32] .sWxSOG_fade{display:none}.sWxSOG_wide{animation:sWxSOG_wide-in .2s var(--ds-ease-in-out)}@keyframes sWxSOG_wide-in{0%{opacity:0}}.sWxSOG_list{min-height:0;margin-left:-4px;margin-right:var(--dsh-session-list-scrollbar-offset);padding-left:4px;padding-right:calc(var(--dsh-session-list-edge-inset) - var(--dsh-session-list-scrollbar-width) - var(--dsh-session-list-scrollbar-offset));scrollbar-gutter:stable;flex:1;padding-bottom:16px;overflow-y:auto}.sWxSOG_flatList>*+*,.sWxSOG_searchTree>[role=treeitem]+[role=treeitem],.sWxSOG_groupSection>*+*{margin-top:2px}.sWxSOG_searchStatus,.sWxSOG_searchWarning{color:var(--dsw-alias-label-tertiary);padding:10px 12px;font-size:12px;line-height:18px}.sWxSOG_searchWarning{color:var(--dsw-alias-label-secondary)}.sWxSOG_groupSection{position:relative}.sWxSOG_groupSection+.sWxSOG_groupSection{margin-top:4px}.sWxSOG_listTopDropIndicator,.sWxSOG_workspaceDropBefore:before,.sWxSOG_workspaceDropAfter:after{content:\"\";z-index:1;background:linear-gradient(55deg, transparent calc(50% - 1px), var(--dsw-alias-state-business-primary) calc(50% - 1px) calc(50% + 1px), transparent calc(50% + 1px)) 0 0 / 5px 7px no-repeat, linear-gradient(125deg, transparent calc(50% - 1px), var(--dsw-alias-state-business-primary) calc(50% - 1px) calc(50% + 1px), transparent calc(50% + 1px)) 0 5px / 5px 7px no-repeat, linear-gradient(var(--dsw-alias-state-business-primary) 0 0) 4px 5px / calc(100% - 4px) 2px no-repeat;pointer-events:none;height:12px;position:absolute;left:0;right:0}.sWxSOG_listTopDropIndicator{top:-8px;left:0;right:var(--dsh-session-list-edge-inset)}.sWxSOG_listTopDropActive>.sWxSOG_workspaceDropBefore:first-child:before{display:none}.sWxSOG_workspaceDropBefore:before{top:-8px}.sWxSOG_workspaceDropAfter:after{bottom:-8px}.sWxSOG_sessionOverflowButton{cursor:pointer;text-align:left;width:100%;height:28px;color:var(--dsw-alias-label-tertiary);background:0 0;border:none;border-radius:8px;padding:0 12px 0 28px;font-size:12px}.sWxSOG_groupSection>.sWxSOG_sessionOverflowButton{margin-top:0}.sWxSOG_sessionOverflowButton:hover{color:var(--dsw-alias-label-secondary);background:0 0}.sWxSOG_empty{color:var(--dsw-alias-label-tertiary);padding:16px 12px;font-size:13px}.sWxSOG_renameInput{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);width:100%;height:44px;color:var(--dsw-alias-label-primary);background:0 0;border-radius:22px;outline:none;padding:7px 14px;font-size:14px;font-weight:400;line-height:22px}.sWxSOG_renameInput:disabled{color:var(--dsw-alias-label-dimmed)}.sWxSOG_renameError{color:var(--dsw-alias-state-error-primary);margin-top:8px;font-size:12px;line-height:18px}.sWxSOG_deleteAction:not(:disabled){color:var(--dsw-alias-state-error-primary)}.sWxSOG_deleteStatus{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px}@media (prefers-reduced-motion:reduce){.sWxSOG_wide{animation:none}.sWxSOG_search,.sWxSOG_sectionLabel,.sWxSOG_searchSlot,.sWxSOG_searchInput,.sWxSOG_headerActions{transition:none}}.sWxSOG_pinnedSection{flex:none;margin-bottom:4px}.sWxSOG_pinnedHeader{height:24px;color:var(--dsw-alias-label-tertiary);align-items:center;gap:4px;padding:0 8px 0 4px;display:flex;font-size:12px;line-height:20px}.sWxSOG_pinnedList>*+*{margin-top:2px}";
		...
		var WorkspaceBrowser_module_css_default = {
			"empty": "sWxSOG_empty",
			"headerActions": "sWxSOG_headerActions",
			"search": "sWxSOG_search",
			"wide": "sWxSOG_wide",
			"headerActionsHidden": "sWxSOG_headerActionsHidden",
			"fade": "sWxSOG_fade",
			"searchExpanded": "sWxSOG_searchExpanded",
			"searchButton": "sWxSOG_searchButton",
			"workspaceDropBefore": "sWxSOG_workspaceDropBefore",
			"list": "sWxSOG_list",
			"sectionHeader": "sWxSOG_sectionHeader",
			"sectionLabel": "sWxSOG_sectionLabel",
			"iconButton": "sWxSOG_iconButton",
			"searchSlotExpanded": "sWxSOG_searchSlotExpanded",
			"deleteStatus": "sWxSOG_deleteStatus",
			"wide-in": "sWxSOG_wide-in",
			"searchStatus": "sWxSOG_searchStatus",
			"rail": "sWxSOG_rail",
			"searchInput": "sWxSOG_searchInput",
			"listTopDropActive": "sWxSOG_listTopDropActive",
			"searchSlot": "sWxSOG_searchSlot",
			"listArea": "sWxSOG_listArea",
			"root": "sWxSOG_root",
			"deleteAction": "sWxSOG_deleteAction",
			"sectionLabelHidden": "sWxSOG_sectionLabelHidden",
			"renameInput": "sWxSOG_renameInput",
			"clearButton": "sWxSOG_clearButton",
			"flatList": "sWxSOG_flatList",
			"searchTree": "sWxSOG_searchTree",
			"listTopDropIndicator": "sWxSOG_listTopDropIndicator",
			"searchWarning": "sWxSOG_searchWarning",
			"sessionOverflowButton": "sWxSOG_sessionOverflowButton",
			"groupSection": "sWxSOG_groupSection",
			"treeBody": "sWxSOG_treeBody",
			"workspaceDropAfter": "sWxSOG_workspaceDropAfter",
			"renameError": "sWxSOG_renameError",
			"pinnedSection": "sWxSOG_pinnedSection",
			"pinnedHeader": "sWxSOG_pinnedHeader",
			"pinnedList": "sWxSOG_pinnedList"
		};
		//#endregion
		...
		*/
		...
		function SessionTree({ useSessions, startSession, open, forkSession, workspaces, archivedSessionIds, onRenameRequest, onDeleteRequest, onSessionRename, onSessionArchive, pinnedSessions, onTogglePin, insertWorkspaceBefore, insertSessionBefore, orderBy, groupExpansion, setGroupExpanded, sessionOrderByAccount, sessionUpdatedAtByAccount, syncSessionOrderAccount, setSessionOrder, t }) {
			...
			const pinnedIds = (0, react.useMemo)(() => new Set(Object.keys(pinnedSessions ?? {})), [pinnedSessions]);
			const pinnedRows = (0, react.useMemo)(() => derivePinned(list, archivedSessionIds, pinnedSessions), [list, archivedSessionIds, pinnedSessions]);
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [(0, react_jsx_runtime.jsx)(PinnedSection, {
							pinnedRows,
							current,
							now,
							open,
							...
							onTogglePin,
							t
						}), groups.length === 0 && pinnedRows.length === 0 && (0, react_jsx_runtime.jsx)("div", {
		...
		function FlatList({ useSessions, open, forkSession, onSessionRename, onSessionArchive, pinnedSessions, onTogglePin, archivedSessionIds, orderBy, sessionOrderByAccount, sessionUpdatedAtByAccount, syncSessionOrderAccount, setSessionOrder, t }) {
			const list = useSessions((s) => s);
			const pinnedIds = (0, react.useMemo)(() => new Set(Object.keys(pinnedSessions ?? {})), [pinnedSessions]);
			const pinnedRows = (0, react.useMemo)(() => derivePinned(list, archivedSessionIds, pinnedSessions), [list, archivedSessionIds, pinnedSessions]);
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [(0, react_jsx_runtime.jsxs)("div", {
					...
					children: [(0, react_jsx_runtime.jsx)(PinnedSection, {
						...
						onTogglePin,
						t
					}), rows.length === 0 && pinnedRows.length === 0 && (0, react_jsx_runtime.jsx)("div", {
						className: WorkspaceBrowser_module_css_default.empty,
						children: t("empty.none")
					}), rows.map((node) => {
